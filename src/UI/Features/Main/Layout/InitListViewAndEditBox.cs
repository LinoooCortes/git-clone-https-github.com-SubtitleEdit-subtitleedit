...
public static partial class InitListViewAndEditBox
{
    ...
    public static Grid MakeLayoutListViewAndEditBox(MainView mainPage, MainViewModel vm)
    {
        ...
        // ── Position panel (collapsed by default) ───────────────────────────────
        var panelPosition = new StackPanel
        {
            Spacing = 0,
            Orientation = Orientation.Vertical,
        ...
        var positionExpander = new Expander
        {
            Header = "Position",
            IsExpanded = false,
            Margin = new Thickness(0, 2, 10, 0),
        };
        positionExpander.Bind(Visual.IsEnabledProperty, new Binding(nameof(vm.SelectedSubtitle))
        {
            Mode = BindingMode.OneWay,
            Converter = notNullConverter,
        });

        var positionInnerPanel = new StackPanel { Spacing = 4 };

        // Vertical alignment toggle: Top / Middle / Bottom
        var vertAlignPanel = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 2 };
        vertAlignPanel.Children.Add(new TextBlock { Text = "V:", VerticalAlignment = VerticalAlignment.Center, Width = 16 });
        foreach (var vertVal in new[] { SubtitleVerticalAlignment.Top, SubtitleVerticalAlignment.Middle, SubtitleVerticalAlignment.Bottom })
        {
            var btn = new ToggleButton { Content = vertVal.ToString(), Tag = vertVal, Padding = new Thickness(4, 2) };
            btn.Bind(ToggleButton.IsCheckedProperty,
                new Binding($"{nameof(vm.SelectedSubtitle)}.{nameof(SubtitleLineViewModel.VerticalAlignment)}")
                {
                    Mode = BindingMode.TwoWay,
                    Converter = new EnumMatchToBoolConverter(),
                    ConverterParameter = vertVal,
                });
            vertAlignPanel.Children.Add(btn);
        }
        positionInnerPanel.Children.Add(vertAlignPanel);

        // Horizontal alignment toggle: Left / Center / Right
        var horizAlignPanel = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 2 };
        horizAlignPanel.Children.Add(new TextBlock { Text = "H:", VerticalAlignment = VerticalAlignment.Center, Width = 16 });
        foreach (var horizVal in new[] { SubtitleHorizontalAlignment.Left, SubtitleHorizontalAlignment.Center, SubtitleHorizontalAlignment.Right })
        {
            var btn = new ToggleButton { Content = horizVal.ToString(), Tag = horizVal, Padding = new Thickness(4, 2) };
            btn.Bind(ToggleButton.IsCheckedProperty,
                new Binding($"{nameof(vm.SelectedSubtitle)}.{nameof(SubtitleLineViewModel.HorizontalAlignment)}")
                {
                    Mode = BindingMode.TwoWay,
                    Converter = new EnumMatchToBoolConverter(),
                    ConverterParameter = horizVal,
                });
            horizAlignPanel.Children.Add(btn);
        }
        positionInnerPanel.Children.Add(horizAlignPanel);

        // Line index (1–23): visible only when LineIndex is non-null
        var lineIndexPanel = new StackPanel
        {
            Orientation = Orientation.Vertical,
            [!Visual.IsVisibleProperty] = new Binding(
                $"{nameof(vm.SelectedSubtitle)}.{nameof(SubtitleLineViewModel.LineIndex)}")
            {
                Converter = notNullConverter,
                Mode = BindingMode.OneWay,
            },
        };
        lineIndexPanel.Children.Add(new TextBlock { Text = "Line (1–23)", FontWeight = FontWeight.Bold });
        lineIndexPanel.Children.Add(UiUtil.MakeNumericUpDownInt(1, 23, 1, double.NaN, vm,
            $"{nameof(vm.SelectedSubtitle)}.{nameof(SubtitleLineViewModel.LineIndex)}"));
        positionInnerPanel.Children.Add(lineIndexPanel);

        // X / Y offset (%)
        var offsetPanel = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 6 };
        var panelOffsetX = new StackPanel { Orientation = Orientation.Vertical };
        panelOffsetX.Children.Add(new TextBlock { Text = "X%", FontWeight = FontWeight.Bold });
        panelOffsetX.Children.Add(UiUtil.MakeNumericUpDownDouble(0.0, 100.0, 0.0, double.NaN, vm,
            $"{nameof(vm.SelectedSubtitle)}.{nameof(SubtitleLineViewModel.OffsetX)}"));
        offsetPanel.Children.Add(panelOffsetX);
        var panelOffsetY = new StackPanel { Orientation = Orientation.Vertical };
        panelOffsetY.Children.Add(new TextBlock { Text = "Y%", FontWeight = FontWeight.Bold });
        panelOffsetY.Children.Add(UiUtil.MakeNumericUpDownDouble(0.0, 100.0, 0.0, double.NaN, vm,
            $"{nameof(vm.SelectedSubtitle)}.{nameof(SubtitleLineViewModel.OffsetY)}"));
        offsetPanel.Children.Add(panelOffsetY);
        positionInnerPanel.Children.Add(offsetPanel);

        positionExpander.Content = positionInnerPanel;
        panelPosition.Children.Add(positionExpander);
        timeControlsPanel.Children.Add(panelPosition);
        // ────────────────────────────────────────────────────────────────────────
